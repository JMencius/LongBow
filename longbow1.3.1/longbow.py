...
def main():
    ...
    if autocorr:
        if pred_dict["Software"] == "dorado" and pred_dict["Mode"] != "FAST":
            train_x, train_y = read_autocorr_train_file(os.path.join(model_path, "dorado_autocorr.csv"))
            ...
            #print("yes")
            train_x, train_y = read_autocorr_train_file(os.path.join(model_path, "guppy_autocorr.csv"))
            hac_sup_pred = predict_hac_sup(autocorr, train_x, train_y)
